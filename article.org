...
#+LATEX_CLASS_OPTIONS: [version=last,draft=false, paper=A4,portrait,twoside=true,twocolumn=false,headinclude=false,footinclude=false,fontsize=12,BCOR=20mm,DIV=calc,pagesize=auto,titlepage=false,abstract=false,mpinclude=true,open=right,captions=tablesignature,captions=figuresignature,numbers=autoendperiod,headsepline=false,parskip=false]
