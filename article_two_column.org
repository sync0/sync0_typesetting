...
#+LATEX_CLASS_OPTIONS: [version=last,draft=false, paper=letter,portrait,twoside=true,twocolumn=true,headinclude=false,footinclude=false,fontsize=11,BCOR=20mm,DIV=calc,pagesize=auto,titlepage=false,abstract=false,mpinclude=false,open=right,captions=tablesignature,captions=figuresignature,numbers=autoendperiod,headsepline=false,parskip=false]
